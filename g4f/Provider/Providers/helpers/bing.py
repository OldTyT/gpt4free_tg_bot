...
def get_token():
    ...
    try:
        cookies = {
            c.name: c.value for c in browser_cookie3.edge(domain_name="bing.com")
        }
        return cookies["_U"]
    except:
...
class AsyncCompletion:
    async def create(
        prompt: str = None, optionSets: list = None, token: str = None
...
async def run(optionSets, messages):
    async for value in AsyncCompletion.create(
        prompt=messages[-1]["content"], optionSets=optionSets
    ):
